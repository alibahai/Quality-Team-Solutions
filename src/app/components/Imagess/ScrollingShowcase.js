  ...
  useEffect(() => {
    const run = (track, dir = -1) => {
      ...
      const step = (ts) => {
        ...
        // content is doubled → total/2 is one full cycle width
        const total = track.scrollWidth / 2;
        if (x <= -total) x += total; // wrap left
        if (x >= 0) x -= total;      // wrap right
    ...
    const stopRow1 = run(row1Ref.current, -1); // left
    const stopRow2 = run(row2Ref.current, +1); // right
    return () => {
      stopRow1?.();
      stopRow2?.();
    };
...
/** Card with three size variants (sm | md | lg), edge-to-edge image inside */
function Card({ size = "md", children }) {
  const base = "relative overflow-hidden rounded-2xl"; // edge-to-edge image, rounded corners
  const sizes = {
